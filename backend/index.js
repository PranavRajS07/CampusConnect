...
app.use(
  cors({
    origin: "https://campus-connect-red.vercel.app",
    credentials: true,
  })
);
...
// Maintain users globally
const users = [];

io.on("connection", (socket) => {
	console.log("socket connected");

	// Listen for the authentication payload
	socket.on("authenticate", (user) => {
		socket.handshake.auth = user; // Store user data on the socket
		users.push({ ...user, socketId: socket.id });
		console.log("Current users:", users);
		io.emit("user-connected", users); // Notify all users of the new connection
	});
	...
	socket.on("disconnect", () => {
		...
		if (index !== -1) {
			...
			socket.to(disconnectedUser.room).emit("user-disconnected", disconnectedUser);
		}
